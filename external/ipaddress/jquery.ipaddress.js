	...
	$.fn.ipaddress = function(params) {
		return $(this).each(function(){
			...
			var refresh = function(){
				...
				$.each($(this).next('.ip_container ip_octet'), function(index){
					$(this).val(ip_value[index]);
				});
			}
			...
			if (!$this.hasClass('ip-enabled')) {
				...
				{
					for (var i = 0; i <=3; i++) {
						octets.push('<select class="ip_octet" id="' + id_prefix + '_octet_' + (i+1) + '" maxlength="3" value="' + ip_value[i] + '" />');
					}
				}
				...
				$this.next('.ip_container').find('.ip_octet').bind('keydown', function(e){
					...
					$this.next().find('input.ip_cidr').bind('keyup', function(e){
			...
			var saveIP = function(el) {
				...
				if ($('input.ip_octet:not(.ip_cidr)', $(el).parent()).filter(function(){ return this.value.length; }).length == 4) {
					...
					$this.val(ip + cidr);
					if(ori_val != ip+cidr) $this.trigger('change');
				}
				else {
					var ori_val = $this.val();
					$this.val('');
					if(ori_val != '') $this.trigger('change');
				}
			}
