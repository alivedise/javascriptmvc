...
(function($){  
	...
	$.fn.jjmenu = function (clickEvent, param, myReplaces, effect, onShow) {  
		...
		function menu(id,param,myReplaces,el,effect) {
			...
			function checkPosition() {
				...
				$(m).css({"top":ntop+effect.offset[1]+"px", "left":nleft+effect.offset[0]+"px"});
			}
			...
			function putItem(n) {
				...
				//custom attr by alive
				if (n.attr) {
                    jQuery(span).attr(n.attr);
                }
				
				$(span).html(uReplace(n.title));
				if(typeof(n.title) == 'undefined') return;
				$(ms).append(item);
			}
			
			/* do action on menu item */
			function doAction(act, event) {
