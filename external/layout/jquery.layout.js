 ...
 */
		...
		var transformData = function( d ) {
			...
			d = d || {};
			if ( d.effects || d.defaults || d.north || d.south || d.west || d.east || d.center ) json = $.extend(json, d); // already in json format - add to base keys
			else
		...
		var execUserCallback = function( pane, v_fn ) {
			...
			try {
				if ( typeof v_fn == "function" ) fn = v_fn;
					...
					args = v_fn.split(","),
						fn = eval(args[0]);
					if ( typeof fn == "function" && args.length > 1 ) return fn(args[1]); // pass the argument parsed from 'list'
				}
		...
		var cssW = function( e, outerWidth ) {
			...
			return outerWidth - cssNum($E, "paddingLeft") - cssNum($E, "paddingRight") - ($.curCSS($E[0], "borderLeftStyle", true) == "none" ? 0 : cssNum($E, "borderLeftWidth")) - ($.curCSS($E[0], "borderRightStyle", true) == "none" ? 0 : cssNum($E, "borderRightWidth"));
		};
		var cssH = function( e, outerHeight ) {
			...
			return outerHeight - cssNum($E, "paddingTop") - cssNum($E, "paddingBottom") - ($.curCSS($E[0], "borderTopStyle", true) == "none" ? 0 : cssNum($E, "borderTopWidth")) - ($.curCSS($E[0], "borderBottomStyle", true) == "none" ? 0 : cssNum($E, "borderBottomWidth"));
		};
 ...
 */
		...
		var create = function() {
			...
			if ( options.scrollToBookmarkOnLoad ) with( self.location ) if ( hash ) replace(hash); // scrollTo Bookmark
			...
			try { // format html/body if this is a full page layout
				if ( $Container[0].tagName == "BODY" ) {
					...
					if (!$Container.hasClass("ui-layout-pane") ) {
						if (!p || "fixed,absolute,relative".indexOf(p) < 0 ) CSS.position = "relative"; // container MUST have a 'position'
						if (!h || h == "auto" ) CSS.height = "100%"; // container MUST have a 'height'
					}
					$Container.css(CSS);
				}
			} catch (ex) {}
		...
		var initOptions = function() {
			...
			$.each(c.borderPanes.split(","), function( i, pane ) {
				...
				$.each(["_open", "_close", ""], function( i, n ) {
					...
					var fxName = o[sName];
					if ( fxName == "none" || !$.effects || !$.effects[fxName] || (!effects[fxName] && !o[sSettings] && !o.fxSettings) ) fxName = o[sName] = "none"; // effect not loaded, OR undefined FX AND fxSettings not passed
...
/*
		...
		var initHandles = function() {
			...
			$.each(c.borderPanes.split(","), function() {
				...
				if ( isOpen && o.resizable ); // this is handled by initResizable
		...
		var initResizable = function() {
			...
			$.each(c.borderPanes.split(","), function() {
				...
				if (!s.isClosed ) $R.attr("title", o.resizerTip).css("cursor", o.resizerCursor) // n-resize, s-resize, etc
				;
 ...
 */
		...
		var show = function( pane, openPane ) {
			...
			else open(pane); // adjust all panes to fit
		};
		...
		var open = function( pane, slide, noAnimation ) {
			...
			function open_2() {
				...
				if ( $R ) {
					...
					else $R.css("cursor", "default"); // n-resize, s-resize, etc
...
/* OLD CODE - keep until sure line above works right!
		...
		var sizeContent = function( panes ) {
			...
			$.each(panes.split(","), function() {
				...
				if ( height < 1 ) $C.hide(); // no room for content!
				else $C.css({
					height: height
				}).show();
			});
		...
		function keyDown(evt) {
			...
			if ( el && SHIFT && isCursorKey && (el.tagName == "TEXTAREA" || (el.tagName == "INPUT" && (code == 37 || code == 39))) ) return true; // allow text-selection
			// SYNTAX NOTES
