...
;(function($) {
	$.widget('naxx.config_cameralog', $.naxx.config, {
		buttons: [
			{
				type: 'settingBtn',
				text: 'Export',
		...
		initTable: function(){
			...
			self.object.$grid = element.find('table.grid').jqGrid({
				url: '/fcgi-bin/dbusproxy.exec_query_surveillance_log',
				mtype: 'POST',
				dataType: 'json',
				altRows: true,
				scrollrows: true,
				...
				datatype: 'json',
				rownumbers: true,
				colNames : ['id' ,'camera' ,'date', 'source', 'content'],
				colModel : [
					{ name : 'id', width : 25, align: 'left', sortable: false, hidden: true},
					{ name : 'camera', width : 30, align: 'left', sortable: false},
					{ name : 'datetime', width : 50, align: 'left', sortable: false},
					{ name : 'source', width : 30, align: 'left', sortable: false},
				...
				beforeProcessing: function(data, status ,xhr){
					$.each(data.rows, function(i, v){
						v.camera = $.naxx.encoder[v.camera].name;
					});
				},
				shrikToFit: true,
				rowNum:20, 
