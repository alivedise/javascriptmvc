...
;(function($) {
	$.widget('naxx.config_systemlog', $.naxx.config, {
		buttons: [
			{
				type: 'settingBtn',
				text: 'Export',
		...
		initTable: function(){
			...
			self.object.$grid = element.find('table.grid').jqGrid({
				url: '/fcgi-bin/dbusproxy.exec_query_system_log',
				mtype: 'POST',
				dataType: 'json',
				altRows: true,
				scrollrows: true,
